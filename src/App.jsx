...
export default function App() {
  ...
  const openDialog = (type) => { dialogTriggerRef.current = document.activeElement; setDialog(type); };
  ...
  return <div className="app-layout"><Sidebar state={state} current={current} dispatch={dispatch} open={drawer} close={() => setDrawer(false)} onRename={() => openDialog('rename')} onDelete={() => openDialog('delete')} />
    <main className={`workspace ${state.mode === 'shop' ? 'shopping' : ''}`}>
      ...
      {state.mode === 'shop' && completed > 0 && <button className="finish button primary" onClick={() => openDialog('finish')}>Einkauf abschließen</button>}
    </main>
