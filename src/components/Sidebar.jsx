...
export function Sidebar({ state, current, dispatch, open, close, onRename, onDelete }) {
  ...
  return <><div className={`drawer-scrim ${open ? 'show' : ''}`} onClick={close} /><aside className={`sidebar ${open ? 'open' : ''}`} aria-label="Listenverwaltung">
    ...
    <div className="sidebar-actions"><button onClick={(event) => onRename(event.currentTarget)}><Edit3 />Umbenennen</button><button onClick={() => dispatch({ type: 'DUPLICATE_LIST' })}><Copy />Duplizieren</button><button className="danger" onClick={(event) => onDelete(event.currentTarget)}><Trash2 />Löschen</button></div>
  </aside></>;
}
